...
function buildRows(data: MatchStat[]) {
  return SUB_FAMILY_ORDER.map((sf) => {
    const sfData = data.filter((d) => d.sub_family === sf)
    if (!sfData.length) return null
  ...
  }).filter(Boolean) as NonNullable<ReturnType<typeof buildRows>[number]>[]
}
...
export default function MatchAverageTables({ data }: Props) {
  ...
  const totalMatches = data.length > 0
    ? Math.max(...SUB_FAMILY_ORDER.map((sf) => {
      ...
      <div>
        <div className="flex flex-wrap items-center gap-4 mb-3">
          ...
          <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5">
            ...
            <button
              onClick={() => setPct((p) => Math.min(200, p + 5))}
              className="w-6 h-6 rounded text-gray-300 hover:bg-gray-700 flex items-center justify-center text-base leading-none"
            >+</button>
